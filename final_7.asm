...
copiaAcadena1_1:			 
	mov al, [ebx+mStr]	 ; IMPRIME NSTR HASTA LLEGAR A FIN DE LINEA (0) Y LEE NUMERO
	mov [ebx+cadena], al
	inc ebx
	cmp al, 0
	jne copiaAcadena1_1
...
seguir1_1:			 ; VALIDA QUE NUMERO INGRESADO SEA MENOR QUE 11
	cmp eax, 11
	jl seguir2_2
	jmp main_1
...
seguir2_2:			 ; MUEVE A N EL VALOR DE EAX (NUMERO)	
	mov [m], eax
...
copiaAcadena3:
	...
	cmp eax, [m] 		 ; COMPARO EL VALOR DE N CON C
...
muestroColumna:
	...
	mov ebx, esi 		 ; GUARDO EN EBX EL VALOR DE ESI (FILA)
	imul ebx, [m] 		 ; MULTIPLICO EBX POR M
	imul ebx, 4		 ; MULTIPLICO EBX POR 4 PORQUE SON QWORDS

	mov edx, edi 		 ; MUEVO A EDX EL VALOR DE EDI (COL)
	...
	jl muestroFila	
        ...
        mov ebx, 0   		; SIGO CON IMPRESION DE MATRIZ ROTADA ANTIHORARIO	
        call mostrarSaltoDeLinea
...
recorroMatrizAntihorario:
	mov esi, dword [c]	 ; CONTADOR COLUMNAS
	dec esi 		 ; ESI-1

muestroFilaAntihorario:
	mov edi, 0 		 ; CONTADOR DE FILAS
...
muestroColumnaAntihorario:
	...
	inc edi 		 ; INCREMENTO EDI Y LO COMPARO CON F
	cmp edi, dword [f]
	jl muestroColumnaAntihorario 

	dec esi 		 ; DECREMENTO ESI Y LO COMPARO CON 0
	call mostrarSaltoDeLinea
	cmp esi, 0 		 
	jge muestroFilaAntihorario	
